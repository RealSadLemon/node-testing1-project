...
describe('[Exercise 1] trimProperties', () => {
  ...
  test('[2] returns a copy, leaving the original object intact', () => {
    const input = { foo: '  foo ', bar: 'bar ', baz: ' baz' }
    const expected = { foo: 'foo', bar: 'bar', baz: 'baz' }
    const finishedCopy = utils.trimProperties(input)
    expect(input).toEqual({ foo: '  foo ', bar: 'bar ', baz: ' baz' })
    expect(finishedCopy).toEqual(expected)
  })
})
...
describe('[Exercise 2] trimPropertiesMutation', () => {
  test('[3] returns an object with the properties trimmed', () => {
    const input = { foo: '  foo ', bar: 'bar ', baz: ' baz' }
    const expected = { foo: 'foo', bar: 'bar', baz: 'baz' }
    const finalObject = utils.trimPropertiesMutation(input)
    expect(finalObject).toEqual(expected)
  })
...
describe('[Exercise 6] Car', () => {
  ...
  test('[15] driving the car returns the updated odometer', () => {
    ...
    expect(focus.drive(100)).toEqual(focus.odometer);
  })
...
describe('[Exercise 7] isEvenNumberAsync', () => {
  test('[19] resolves true if passed an even number', () => {
    utils.isEvenNumberAsync(4).then(result => {
      expect(result).toEqual(true)
    })
  })
  test('[20] resolves false if passed an odd number', () => {
    utils.isEvenNumberAsync(43567).then(result => {
